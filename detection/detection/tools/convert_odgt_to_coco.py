...
def convert_odgt2coco(odgtpath, targetclass):
    ...
    def add_anno_item(object_name, image_id, category_id, tmp_bbox, annotation_id, w, h):
        bbox =[]
        bbox.append(tmp_bbox["tl"]["x"] * w)
        bbox.append(tmp_bbox["tl"]["y"] * h)
        bbox.append((tmp_bbox["br"]["x"] - tmp_bbox["tl"]["x"]) * w)
        bbox.append((tmp_bbox["br"]["y"] - tmp_bbox["tl"]["y"]) * h)
    ...
    itemlist = []
    file = open(odgtpath, 'r', encoding='utf-8')
    for line in file.readlines():
        dic = json.loads(line)
        itemlist.append(dic)
    file.close()
    
    print("aaa: " + str(len(itemlist)))
    
    for i, lineinfo in enumerate(itemlist):
        item = lineinfo
        print(i)
        #print(item)
        item_id = item["label_entity"]["resources"][0]["id"]
        filename = '{}.jpg'.format(item_id)
        size = dict()
        size['width'] = item["label_entity"]["resources"][0]["size"]["width"]
        size['height'] = item["label_entity"]["resources"][0]["size"]['height']
        size['depth'] = 3

        add_img_item(filename, size, item_id, i)

        if item["label_entity"]["groundtruth"]["elements"] is not None:
            for bbox in item["label_entity"]["groundtruth"]["elements"]:
                object_name = "chongqigongmen"
                ...
                bbox = bbox['rect']
                annotation_id = add_anno_item(object_name, i, current_category_id, bbox, annotation_id, size['width'], size['height'] )
...
def main():
    ...
    savepath = "/data/jupyter/data/result/nori/ret.json"
